...
contract ERC20 is IERC20 {
   ...
   constructor(string memory name_, string memory symbol_, uint initalSupply, address shop){
    _name = name_;
    _symbol = symbol_;
    owner = msg.sender;
    //ввод в обород токенов
    mint(initalSupply, shop);
   }
   ...
   function _approve(address sender, address spender, uint amount) internal virtual{
    ...
    emit Aprove(sender, spender, amount);
   }

   function transferFrom(address sender, address recipiend, uint amount) external enoughTokens(sender,amount){
    _beforeTokenTransfer(sender,recipiend,amount);    
    // require(allowances[sender][recipiend] >= amount, "check allowance!");
    allowances[sender][recipiend] -=amount;

    balances[sender] -= amount;
    balances[recipiend] += amount;

    emit Transfer(sender, recipiend, amount);
   }
